...
class ApiService {
  ...
  static Future<Map<String, dynamic>> getMe() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return {
          'success': false,
          'error': data['error'] ?? 'Lỗi lấy thông tin người dùng',
        };
      }
    } catch (e) {
      return {'success': false, 'error': 'Lỗi kết nối: ${e.toString()}'};
  ...
  // Weather API Methods
  static String get weatherApiKey => ApiConfig.weatherApiKey;
  static String get weatherBaseUrl => ApiConfig.weatherBaseUrl;
  ...
  static Future<Map<String, dynamic>> getCurrentWeather(
    ...
    try {
      final response = await http.get(
        Uri.parse(
          '$weatherBaseUrl/weather?lat=$lat&lon=$lon&appid=$weatherApiKey&units=metric&lang=vi',
        ),
      );
      ...
      return {
        'success': false,
        'error': 'Lỗi kết nối thời tiết: ${e.toString()}',
      };
    }
  }
  ...
  static Future<Map<String, dynamic>> getForecast(
    ...
    try {
      final response = await http.get(
        Uri.parse(
          '$weatherBaseUrl/forecast?lat=$lat&lon=$lon&appid=$weatherApiKey&units=metric&lang=vi',
        ),
      );
  ...
  static Future<Map<String, dynamic>> searchLocations(String query) async {
    try {
      ...
      return {
        'success': false,
        'error': 'Lỗi kết nối tìm kiếm: ${e.toString()}',
      };
    }
  }
}
