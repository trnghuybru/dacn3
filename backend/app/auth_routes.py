...
def _is_phone_blocked(conn, phone: str) -> bool:
    with conn.cursor() as cur:
        cur.execute("SELECT 1 FROM blocked_numbers WHERE phone = %s LIMIT 1", (phone,))
...
def _user_by_phone(conn, phone: str) -> dict | None:
    with conn.cursor() as cur:
        cur.execute(
            "SELECT id, phone, full_name, device_token, password_hash, district_id, status, created_at "
...
def register():
    ...
    lat, lng (optional, mặc định 0,0 cho last_known_location), district_id (optional).
    """
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                """
                INSERT INTO users (phone, full_name, device_token, password_hash, last_known_location, district_id, status)
                VALUES (%s, %s, %s, %s, ST_GeomFromText(%s, 4326), %s, 'ACTIVE')
                """,
                (phone, full_name, device_token, password_hash, wkt_point, district_id),
            )
            conn.commit()
...
def me():
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT id, phone, full_name, device_token, district_id, status, created_at "
